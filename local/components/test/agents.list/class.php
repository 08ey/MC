...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function executeComponent(): void
    {
        ...
        if ($this->cache->initCache(
            ...
            $arTypeAgents = self::getFieldListValue($arHlblock, 'UF_TYPE_OF_ACT'); // получить массив со значениями списочного свойства Виды деятельности агентов
         ...
         */ 
         $this->arResult['STAR_AGENTS'] = CUserOptions::GetOption("agent", "agents_star");
        /*
    ...
    private function getFieldListValue(array $arHlblock, string $fieldName): array
    {
        ...
        if ($fieldID) {
			$entity = CUserFieldEnum::GetList(array(), array("USER_FIELD_ID"=>$fieldID));
			while($field=$entity->fetch()){
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $nav->allowAllRecords(true)
            ->setPageSize($this->arParams['ELEMENT_COUNT']) //Нужно передать параметр Количество элементов из мввсива $this->arParams
            ->initFromUri();
        $rsAgents = $entity::GetList([
				"filter" => array("UF_ACT"=>"1"),
				"count_total" => true,
        ...
        while ($arAgent = $rsAgents->fetch()) {
			$arAgent['UF_TYPE_OF_ACT']=$arTypeAgents[$arAgent['UF_TYPE_OF_ACT']];
    ...
    public function clickStarAction($agentID)
    {
		...
		$ar=[];
		$userOption = CUserOptions::GetOption("agent", "agents_star");
		if(isset($userOption)){
		...
		if(CUserOptions::SetOption("agent", "agents_star", $value)){$result["action"]='success';};
        /*
